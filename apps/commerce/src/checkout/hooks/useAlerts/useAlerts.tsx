...
function useAlerts(globalScope?: any): any {
	...
	const getMessageKey = ({ scope, field, code }: IAlertErrorData, { error } = { error: false }) => {
		const keyBase = `${scope}-${field}-${code}`;
		return camelCase(error ? `${keyBase}-error` : keyBase);
	};

	const getErrorMessage = useCallback(({ code, field, scope }: IAlertErrorData): string => {
	...
	const getParsedAlert = useCallback(
		(data: IAlertErrorData, type: AlertType): IAlert => {
	...
	const showAlert = useCallback(
		({ message, type = 'error', ...rest }: Pick<IAlert, 'message'> & { type?: AlertType; id?: string }) =>
			toast(<p>{message}</p>, { type, ...rest }),
		[],
	);

	const showDefaultAlert = useCallback(
		(alertErrorData: IAlertErrorData, { type }: { type: AlertType } = { type: 'error' }) => {
	...
	const showErrors = useCallback(
		(errors: ApiErrors<any>, scope: CheckoutScope = globalScope) =>
			getParsedApiErrors(errors).forEach(error => showDefaultAlert({ ...error, scope } as IAlertErrorData)),
